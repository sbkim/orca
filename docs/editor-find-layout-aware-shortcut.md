# Layout-aware find in the source editor
...
The source editor currently installs Orca's layout-aware save shortcut through `editor-shortcuts.ts` (`src/renderer/src/components/editor/editor-shortcuts.ts:18`), but find is left entirely to Monaco's internal keycode dispatch (`src/renderer/src/components/editor/MonacoEditor.tsx:393`). Orca already declares `editor.find` as `Mod+F` (`src/shared/keybindings.ts:784`) and its matcher resolves logical keys before physical-code fallback (`src/shared/keybindings.ts:2041`).
...
Monaco's built-in find keybinding follows the physical/virtual keycode delivered by Chromium. Orca's shortcut system is layout-aware, but the Monaco source editor does not use it for find. On layouts where the key that produces `f` is not physical `KeyF`, Monaco misses the chord and Native Edit Context remains in editing mode.
...
- macOS/Linux/Windows keydown reaches the Monaco container.
...
- A removed/disposed editor must not retain the listener.
...
- Unit: `editor-shortcuts.test.ts` dispatches keyboard events through a real element and parameterizes layout-aware matching across macOS (`metaKey`) and Linux/Windows (`ctrlKey`) using logical `f` with physical `KeyU`. It also asserts QWERTY/default handling, matched-repeat prevention without a second callback, unrelated typing, and disposal behavior.
...
3. Wire it to Monaco's existing find action and lifecycle cleanup.
4. Run focused tests, typecheck, lint, and the Electron QWERTY/layout/typing scenarios.

## Lightweight Eng Review

- Scope: Kept to one source-editor shortcut installer, one Monaco hookup, and regression coverage; no new find implementation or global shortcut interception.
...
- Performance/blast radius: One capture listener per mounted source editor, doing a constant-time keybinding comparison only on keydown. It is removed with Monaco disposal; no polling, scans, IPC, storage, or render-loop work.

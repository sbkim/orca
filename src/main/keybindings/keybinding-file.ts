...
function normalizeBindingValue(
  ...
  if (typeof value === 'string') {
    const normalized = normalizeKeybindingList(value)
  ...
  if (Array.isArray(value)) {
    ...
    const normalized = normalizeKeybindingArray(value)
    return Array.isArray(normalized)
      ? { ok: true, value: normalized }
      : normalized.ok
        ? { ok: true, value: [normalized.value] }
...
function normalizeWriteBindingValue(value: unknown): string[] | null {
  if (value === null) {
    return null
  }
...
export function writeKeybindingOverride(
  ...
  const normalizedBindings = normalizeWriteBindingValue(bindings)

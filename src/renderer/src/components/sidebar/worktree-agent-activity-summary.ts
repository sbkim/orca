...
function getWorktreeAgentActivitySummaries(
  ...
  // Why: any agentStatusEpoch bump (an UNRELATED worktree's transition, or a
  // freshness tick) invalidates this shared cache and rebuilds every worktree's
  // summary with fresh objects/Sets. Each sidebar card subscribes to its own
  // summary via useShallow, so a new nested agentStatusPaneIdsByTabId ref would
  // re-render every card even when its own agent state is unchanged. Reuse the
  // prior summary object when structurally equal so only transitioning cards
  // re-render.

...
export function revealElementInScrollContainer(
  ...
  if (nextScrollTop !== null) {
    // Why: honor the user's reduced-motion preference by jumping instantly
    // instead of animating a smooth scroll (also makes the reveal deterministic
    // in headless environments that never tick the smooth-scroll animation).
